			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="created" title="${message(code: 'tweet.created.label', default: 'Created')}" />
					
						<g:sortableColumn property="location" title="${message(code: 'tweet.location.label', default: 'Location')}" />
				...
				<g:each in="${tweetInstanceList}" status="i" var="tweetInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><tweet:formatDate date="${tweetInstance.created}" /></td>
					
						<td>${fieldValue(bean: tweetInstance, field: "location")}</td>
					
						<td><tweet:showAuthor instance="${tweetInstance.author}" /></td>
